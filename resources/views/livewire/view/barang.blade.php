<section class="w-full h-auto flex justify-center items-center flex-col mt-8 gap-[25px]">
    ...
    <div class="w-full h-auto p-4 absolute flex justify-center items-center px-8 {{$this->toggleTesti == false ? 'hidden' : 'block'}}"
        id="popupTesti">
...
for (let i = 1; i <= 5; i++) {
    ...
    star.addEventListener('click', function() {
        const cek = document.getElementById('ratingInput').value = i;
        // Remove existing orange color from all stars
        for (let j = 1; j <= 5; j++) {
            if (j <= i) {
